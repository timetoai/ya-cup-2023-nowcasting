...
# same as 1 but with radial_velocity and reflectivity
class RadarDataset2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for timestamp in self.sequences[index]:
            with h5py.File(self.timestamp_to_file[timestamp]) as d:
                data.append(np.concatenate([np.expand_dims(np.array(d[timestamp]['intensity']), 0),
                                            np.array(d[timestamp]["radial_velocity"]),
                                            np.array(d[timestamp]["reflectivity"])]))
        data = np.stack(data)
        data[data == -1e6] = 0
        data[data == -2e6] = -1
        inputs = data[:self.in_seq_len]
        targets = data[self.in_seq_len:, 0: 1]
        if self.with_time:
            return (inputs, self.sequences[index][-1]), targets
        else:
            return inputs, targets
